...
import 'finance_purchase_invoice_detail_screen.dart';
import 'finance_purchase_invoice_form_screen.dart';

class FinancePurchaseInvoicesScreen extends StatefulWidget {
  const FinancePurchaseInvoicesScreen({
    super.key,
  ...
  @override
  State<FinancePurchaseInvoicesScreen> createState() =>
      _FinancePurchaseInvoicesScreenState();
}

class _FinancePurchaseInvoicesScreenState
    extends State<FinancePurchaseInvoicesScreen> {
  ...
  Future<void> _openForm({FinancePurchaseInvoice? invoice}) async {
    ...
    if (!FinancePermissions.canViewFinanceInvoices(
      ...
      return FinanceScaffold(
        assistantContext: FinanceAssistantContextFactory.fromCompany(
          context: context,
          companyData: widget.companyData,
          screenKey: FinanceAssistantScreens.purchaseInvoicesList,
          tabKey: FinanceAssistantTabs.invoices,
          tabLabelKey: 'help_invoices_tab_title',
        ),
        appBar: AppBar(
          title: Text(FinanceStrings.t(context, 'purchase_invoices_title')),
        ),
    ...
    return FinanceScaffold(
      assistantContext: FinanceAssistantContextFactory.fromCompany(
        ...
        actions: FinanceAssistantContextFactory.createAndRefresh(
          createKey: 'purchase_invoice_new',
      ...
      appBar: AppBar(
        ...
        actions: [
          if (_canCreate)
            IconButton(
              tooltip: FinanceStrings.t(context, 'purchase_invoice_new'),
              ...
              items: [
                ...
                ...FinanceDisplayLabels.invoiceStatusCodes.map(
                  (code) => DropdownMenuItem(
                    value: code,
                    child: Text(
                      FinanceDisplayLabels.invoiceStatus(context, code),
                    ),
                  ),
                ),
              ],
