...
class ProductionProcessEditScreen extends StatefulWidget {
  final Map<String, dynamic> companyData;
  final String processId;
  final String plantKey;

  const ProductionProcessEditScreen({
    super.key,
  ...
  @override
  State<ProductionProcessEditScreen> createState() =>
      _ProductionProcessEditScreenState();
}

class _ProductionProcessEditScreenState extends State<ProductionProcessEditScreen> {
  ...
  Future<void> _bootstrap() async {
    ...
    try {
      ...
      if (p == null) {
        setState(() {
          _pageLoading = false;
          _loadError = 'Proces nije pronađen.';
        });
        return;
      }
      _existing = p;
      _plantLabel = pl;
      _codeCtrl.text = p.processCode;
      _nameCtrl.text = p.name;
      _descriptionCtrl.text = p.description;
      _processType = p.processType.isEmpty
          ? ProductionProcess.typeMachining
          : p.processType;
      _status = p.status;
      _iatf = p.iatfRelevant;
      _trace = p.traceabilityRequired;
      _qc = p.qualityControlRequired;
      _firstPiece = p.firstPieceApprovalRequired;
      _params = p.processParametersRequired;
      _operatorQual = p.operatorQualificationRequired;
      _wiReq = p.workInstructionRequired;
      _pfmeaReq = p.pfmeaRequired;
      _cpReq = p.controlPlanRequired;
      _wcTypeKeys
        ..clear()
        ..addAll(p.linkedWorkCenterTypes);
      _linkedWcIds
        ..clear()
        ..addAll(p.linkedWorkCenterIds);
      _pfmeaRefCtrl.text = p.pfmeaReference;
      _cpRefCtrl.text = p.controlPlanReference;
      _wiRefCtrl.text = p.workInstructionReference;
  ...
  Future<void> _save() async {
    ...
    if (existing.isArchived) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Arhivirani proces se ne može uređivati.')),
      );
      return;
    }
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Proces je ažuriran.')),
      );
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    if (_loadError != null) {
      return Scaffold(
        appBar: AppBar(title: const Text('Uredi proces')),
        body: Center(child: Text(_loadError!)),
      );
    }

    final archived = _existing?.isArchived == true;

    return Scaffold(
      appBar: AppBar(
        title: const Text('Uredi proces'),
        actions: [
          TextButton(
        ...
        child: ListView(
          ...
          children: [
              ...
              Padding(
                ...
                child: Material(
                  ...
                  child: const Padding(
                    ...
                    child: Text(
                      'Proces je arhiviran. Detalji su samo za čitanje.',
                    ),
                  ),
                ),
              ),
            ListTile(
              ...
              subtitle: Text(
                _plantLabel.isEmpty ? widget.plantKey.trim() : _plantLabel,
              ),
            ),
            ...
            TextFormField(
              ...
              validator: archived ? null : (v) => _required(v, 'Šifra procesa'),
            ),
            ...
            TextFormField(
              ...
              validator: archived ? null : (v) => _required(v, 'Naziv'),
            ),
            const SizedBox(height: 12),
            ...
            DropdownButtonFormField<String>(
              ...
              items: ProductionProcess.selectableStatuses
                  .where(
                    (e) =>
                        e.key != ProductionProcess.statusArchived ||
                        _status == ProductionProcess.statusArchived,
                  )
                  .map(
                    (e) => DropdownMenuItem(value: e.key, child: Text(e.value)),
                  )
            ...
            SwitchListTile(
              ...
              value: _iatf,
              onChanged: archived ? null : (v) => setState(() => _iatf = v),
            ),
            SwitchListTile(
              contentPadding: EdgeInsets.zero,
              title: const Text('Obavezna sljedljivost'),
              value: _trace,
              onChanged: archived ? null : (v) => setState(() => _trace = v),
            ),
            SwitchListTile(
              contentPadding: EdgeInsets.zero,
              title: const Text('Obavezan QC zapis'),
              value: _qc,
              onChanged: archived ? null : (v) => setState(() => _qc = v),
            ),
            const SizedBox(height: 8),
            const Text(
              'Preporučena pravila',
            ...
            SwitchListTile(
              ...
              value: _firstPiece,
              onChanged: archived ? null : (v) => setState(() => _firstPiece = v),
            ),
            SwitchListTile(
              contentPadding: EdgeInsets.zero,
              title: const Text('Obavezni procesni parametri'),
              value: _params,
              onChanged: archived ? null : (v) => setState(() => _params = v),
            ),
            SwitchListTile(
              contentPadding: EdgeInsets.zero,
              ...
              onChanged: archived
                  ? null
                  : (v) => setState(() => _operatorQual = v),
            ),
            SwitchListTile(
              contentPadding: EdgeInsets.zero,
              title: const Text('Radna instrukcija obavezna'),
              value: _wiReq,
              onChanged: archived ? null : (v) => setState(() => _wiReq = v),
            ),
            SwitchListTile(
              contentPadding: EdgeInsets.zero,
              title: const Text('PFMEA obavezan'),
              value: _pfmeaReq,
              onChanged: archived ? null : (v) => setState(() => _pfmeaReq = v),
            ),
            SwitchListTile(
              contentPadding: EdgeInsets.zero,
              title: const Text('Control Plan obavezan'),
              value: _cpReq,
              onChanged: archived ? null : (v) => setState(() => _cpReq = v),
            ),
            const SizedBox(height: 16),
            const Text(
              'Tipovi radnih centara',
              style: TextStyle(fontWeight: FontWeight.w700),
            ),
            const SizedBox(height: 8),
            Wrap(
            ...
            Row(
              children: [
                ...
                OutlinedButton(
                  onPressed: archived || _wcLoading ? null : _pickWorkCenters,
            ...
            if (!archived)
              FilledButton.icon(
                ...
                label: Text(_saving ? 'Spremanje…' : 'Spremi izmjene'),
              ),
          ],
        ),
      ),
